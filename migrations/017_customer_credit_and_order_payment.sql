...
ALTER TABLE `customer`
  ADD COLUMN `credit_days`    INT          NOT NULL DEFAULT 0
             COMMENT 'Credit term days: 0=COD, 7=weekly, 30=monthly, ...' AFTER `credit_level`,
  ADD COLUMN `payment_terms`  VARCHAR(32)
             COMMENT 'Payment terms label: COD / Weekly / Monthly / Net 30 ...' AFTER `credit_days`;
...
ALTER TABLE `sales_order`
  ...
  ADD COLUMN `paid_amount`    DECIMAL(14,2) NOT NULL DEFAULT 0
             COMMENT 'Cumulative received amount (functional currency KES)' AFTER `payment_status`,
  ADD COLUMN `due_date`       DATE
             COMMENT 'Due date = order_date + customer.credit_days' AFTER `paid_amount`;
